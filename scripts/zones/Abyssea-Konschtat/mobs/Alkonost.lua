...
function onMobDeath(mob,killer)
	...
	if (randomnumber >= 50) and (killer:hasKeyItem(TATTERED_HIPPOGRYPH_WING) == false) then
		killer:addKeyItem(TATTERED_HIPPOGRYPH_WING);
		killer:messageSpecial(KEYITEM_OBTAINED,TATTERED_HIPPOGRYPH_WING);
	end
	if (redtrigdrops >= 1) and (killer:hasKeyItem(ATMA_OF_GALES) == false) then
		if (redtrigdrops == 1) then
			killer:addKeyItem(ATMA_OF_GALES);
			killer:messageSpecial(KEYITEM_OBTAINED,ATMA_OF_GALES);
